...
##### Check the learner Documentation
...
The learner must implement this architecture:
...
> The `billing-db-0` is the ID of the pods running the billing db database - it might be different. To check which is the pods ID run `kubectl get pods | grep billing-db`

###### Can you confirm the `billing` database is listed?

##### Still in `psql` run `\c billing_db` and then `TABLE orders;`.
...
###### Can you confirm the `billing-app` container was correctly started?
...
In less than 15 minutes and with the help of Google the learner must explain all Kubernetes components and their roles.

###### Can the learner explain the K8s components in less than 15 minutes?

#### Bonus

###### +Did the learner used his/her own `play-with-container` solution instead of the provided one?
